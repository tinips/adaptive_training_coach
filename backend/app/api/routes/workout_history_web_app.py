...
from app.api.routes.telegram_web_app import telegram_web_app_identity
...
async function load(append){$('#error').classList.add('hidden');const range=currentRange();if(!range.start_date||!range.end_date||range.start_date>range.end_date){$('#error').textContent='Choose a valid start and end date.';$('#error').classList.remove('hidden');return}if(!webApp?.initData){$('#error').textContent='Open Workout history from Telegram.';$('#error').classList.remove('hidden');return}const payload={init_data:webApp.initData,...range,discipline:state.discipline,cursor:append?state.cursor:null};try{const response=await fetch('/webapp/workout-history/data',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!response.ok)throw new Error();render(await response.json(),append)}catch(_){$('#error').textContent='Unable to load your workout history. Please try again.';$('#error').classList.remove('hidden')}}
...
async def workout_history_web_app() -> str:
    return _PAGE
