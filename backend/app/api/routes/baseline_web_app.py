...
const config={
 ...
 'running.recent_race_effort':{section:'Running',label:'How hard was that run?',kind:'choice',optional:true,options:[['MAXIMAL','Maximal / race effort'],['HARD','Hard effort'],['STEADY','Steady effort'],['EASY','Easy effort']],help:'Optional. Tap the closest effort; this keeps a hard result from being treated as an easy training pace.'},
...
async def submit_baseline(request: Request) -> dict[str, bool]:
    ...
    if result.kind == "baseline_validation_error":
        raise HTTPException(status_code=422, detail=result.error_code)
    if result.kind != "onboarding_completed":
        raise HTTPException(status_code=409, detail="baseline submission is stale")
    saved_message = (
