...
_FALLBACK_VOLUME_METERS_PER_MINUTE = {
...
def _fallback_distance_range_meters(
    *, discipline: Discipline, duration_minutes: int, intensity: dict[str, object]
    ...
    """Return a conservative volume range for every endurance fallback.

    Pace-based sessions are calculated from the pace range. RPE-only swimming
    and cycling still need a visible volume, so they use a deliberately broad
    duration-based estimate only on this provider-failure fallback path.
    """
    ...
    if unit_meters is not None:
        ...
        distance_meters = (
            duration_minutes * _FALLBACK_VOLUME_METERS_PER_MINUTE[discipline]
        )
