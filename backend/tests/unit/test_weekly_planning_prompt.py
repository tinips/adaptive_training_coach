...
def test_first_week_prompt_is_a_probe_and_has_no_goal_payload() -> None:
    ...
    assert FIRST_WEEK_PLANNER_PROMPT_VERSION == 19
    ...
    assert "selected effort value" in str(messages[0].content)
    assert "performance ceiling, not a literal easy-training pace" in str(
        messages[0].content
    )
...
def test_first_week_prompt_requests_only_evaluator_evidence() -> None:
    system = str(
        build_first_week_planner_messages({"planner_mode": "FIRST_WEEK"})[0].content
    )

    assert "record actual day/time, duration" in system
    assert "available\npace/power/heart-rate" in system
    assert "duration, RPE, how it felt" not in system
