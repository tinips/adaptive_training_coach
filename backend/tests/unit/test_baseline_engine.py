...
def test_one_discipline_does_not_fabricate_metrics_for_another() -> None:
    ...
    run = next(
        item for item in calculation.disciplines if item.discipline == Discipline.RUN
    )
    swim = next(
        item for item in calculation.disciplines if item.discipline == Discipline.SWIM
    )
...
def test_activity_crossing_midnight_keeps_full_duration() -> None:
    ...
    run = result_for([crossing], Discipline.RUN)
...
def test_active_weeks_longest_session_and_recent_count_are_exact() -> None:
    ...
    run = result_for(activities, Discipline.RUN)
...
def test_confidence_increases_with_observations_and_stale_data_reduces_it() -> None:
    ...
    sparse_result = result_for(sparse, Discipline.RUN)
    recent_result = result_for(sufficient_recent, Discipline.RUN)
    stale_result = result_for(sufficient_stale, Discipline.RUN)
...
def test_central_provisional_thresholds_can_reach_advanced() -> None:
    ...
    run = result_for(sustained_volume, Discipline.RUN)
...
class ActivityRepositoryFake:
    def __init__(self, records: list[ActivityRecordFake]) -> None:
